...
def Qanalyze(userData, difdata, selection, instrParams, autoremove, BGstrip):
    ...
    if BGstrip:
        ...
        BG = BGsmooth_and_strip_rdmstep(stripwindow, stripwindow/5, stripiterations, BGsmoothing)
    else:
    ...
    starttime = time.time()
    DB, RIR, peakcount = makeDB(difdata, Lambda) #DB, RIR, peakcount = makeDB(difdata, code, enable, Lambda)
    DB2T = DB[:,:,0]
    DBInt = DB[:,:,1]
    ...
    if initialize and autoremove:
        ...
        if sum(enable) > 0:        
            code, RIR, enable, Thresh, Iinit, PatDB = CleanMineralListPatDB(Iinit)
...
def BGsmooth_and_strip_rdmstep(stripwindow, rdmfactor, iterations, smooth):   
    ...
    if smooth:
        datain = smoothdata(diff, SavitzkyGolay_window, SavitzkyGolay_poly_order)
    else:
        datain = diff
    ...
    while Keep_refining:
        ...
        if autoremove:
            ...
            if sum(enable2) < sum(enable):
                Keep_refining = True
                enable = enable2
                mineral, RIR, enable, Thresh, I, PatDB = CleanMineralListPatDB(I)
